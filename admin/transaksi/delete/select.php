<?php
    ...
    if (isset($_POST['kategori_id']) && !empty($_POST['kategori_id'])) {
        ...
        $query = "SELECT id_gambar, nama_gambar FROM gambar WHERE id_kategori_gambar = '$id'";
    ...
    if (isset($_POST['id_barang']) && !empty($_POST['id_barang'])) {
        ...
        // Fetch state name base on country id
        $id = $_POST['id_barang'];
        $query = "SELECT gambar_brg FROM barang WHERE id_barang = '$id'";
        $result = mysqli_query($konek, $query);
        
        if(mysqli_num_rows($result) > 0){
            while ($row = mysqli_fetch_assoc($result)) {
                echo '<img src="../assets/img/desain/'.$row['gambar_brg'].'" class="rounded mx-auto d-block" width="200px" height="200px" alt="'.$row['gambar_brg'].'">';
            }
        }else {
            echo '<p>Image not available</p>';
        }
