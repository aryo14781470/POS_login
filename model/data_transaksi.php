<?php
	...
	function delete_cart_order(){
		$konek = conn();
		$id_cart = $_POST['id_cart'];
		$sql = "DELETE FROM cart_order WHERE id_cart = '$id_cart'";
		$result = mysqli_query($konek, $sql);
		if(!$result){
	...
	function update_qty_detail_cart_order(){
		...
		$cek_qty =  mysqli_fetch_all(viewBarang($id_barang), MYSQLI_ASSOC);
		foreach ($cek_qty as $row){
			$qty_brg = $row['jumlah_brg'];
		}
		if ($qty > $qty_brg){
			$_SESSION['alert'] = dangerAlert("Danger! ","QTY melebihi stok");
		}else{
			...
			if (!$result){
				...
				$_SESSION['alert'] = successAlert("Success! ","QTY berhasil ditambahkan");
			}
		}
		reload();
	}
	...
	function tambah_transaksi(){
		...
		if (!$result) {
			...
			delete_cart_order();
			$lokasi = "http://localhost/POS_Login/admin/index.php?page=Status_Order";
...
//		$jumlah = $row_jumlah['jumlah'];
		...
		if ($jumlah > $grand_total || $jumlah == $grand_total){
			...
			if(!$result_status){
				...
				$rows = mysqli_fetch_all(view_transaksi_detail($id_transaksi), MYSQLI_ASSOC);
				foreach ($rows as $row){
					$id_barang = $row['id_barang'];
					$qty = $row['detail_qty'];
					$sql = "UPDATE items SET jumlah_brg = jumlah_brg - '$qty' WHERE id_barang = '$id_barang'";
					$result = mysqli_query($konek, $sql);
					if (!$result){
						$error = mysqli_error($konek);
						$_SESSION['alert'] = warningAlert("Warning! ", "Detail gagal disimpan. ($error)");
					}else{
...
//		foreach ($total as $row){ $subtotal += $row[0]; }
		...
		$ongkir = $_POST['ongkir'];
		$diskon = $_POST['diskon'];
		$grand_total = $subtotal + $ongkir - $diskon;
		...
		if ($exist > 0){
			...
			$sql_next_paymnet = "INSERT INTO transaksi_payment (id_transaksi, total, ongkir, diskon, grand_total, bayar, berkas, metode, pembayaran, reference, dibuat_tanggal)
                        VALUE ('$id_transaksi', '$subtotal', '$ongkir', '$diskon', '$grand_total', '$bayar', '$name', '$payment', '$max', '$reference', NOW())";
			...
			$sql_paymnet = "INSERT INTO transaksi_payment (id_transaksi, total, ongkir, diskon, grand_total, bayar, berkas, metode, pembayaran, reference, dibuat_tanggal)
                        VALUE ('$id_transaksi', '$subtotal', '$ongkir', '$diskon', '$grand_total', '$bayar', '$name', '$payment', 1, '$reference', NOW())";
